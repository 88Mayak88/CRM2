...
function rowToEvent(r) {
  return { id:r.id, title:r.title||"", date:r.date||"", source:r.source||"",
    ...
    notes:r.notes||"", createdAt:r.created_at||"", archived:r.archived||false };
}
function eventToRow(e) {
  return { id:e.id, title:e.title, date:e.date, source:e.source,
    ...
    notes:e.notes, created_at:e.createdAt, archived:e.archived||false };
}
async function dbLoad() {
  const res = await fetch(`${SB_URL}/rest/v1/events?order=date.asc`, { headers:H });
...
async function dbDelete(id) {
  const res = await fetch(`${SB_URL}/rest/v1/events?id=eq.${id}`, { method:"DELETE", headers:H });
  if (!res.ok) throw new Error(await res.text());
}
...
function InfoCard({ label, value, accent, children }) {
  const ex = accent==="red"?{borderColor:"#F5C6C2",background:C.redBg}
    :accent==="green"?{borderColor:"#A8D9BB",background:C.greenBg}:{};
    ...
    <div style={s.tabBar}>
      {tabs.map(t=>(
        <button key={t.id} style={{...s.tabBtn,...(view===t.id?s.tabActive:{})}} onClick={()=>onTab(t.id)}>
          ...
          <span style={{fontSize:9,fontWeight:700,letterSpacing:0.3,marginTop:1,
            color:view===t.id?C.accent:C.textMuted}}>{t.label}</span>
        </button>
      ))}
    </div>
  );
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {clients.length===0&&(
          <div style={s.emptyState}>
            ...
            <div style={{color:C.textMuted,fontSize:13}}>Клиенты появятся автоматически<br/>когда добавишь мероприятия</div>
          </div>
        )}
        {clients.map((c,i)=>(
          <div key={i} style={s.card}>
            ...
            {c.phone&&(
              <a href={`tel:${c.phone}`} style={{fontSize:14,color:C.accent,fontWeight:600,textDecoration:"none",display:"block",marginBottom:6}}>
                {c.phone}
              </a>
            )}
            <div style={s.cardTags}>
              {c.events.slice(0,3).map(ev=>(
                <Tag key={ev.id}>{ev.title||"Без названия"}</Tag>
              ))}
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",borderBottom:`1px solid ${C.border}`}}>
        ...
        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:2,color:C.accent}}>
      ...
      <div style={s.calGrid}>
        {cells.map((d,i)=>{
          ...
          return (
            <div key={i} style={{
              ...s.calCell,
              ...(isToday?{background:C.accent,borderRadius:20}:{}),
              ...(isSel&&!isToday?{background:C.tagBg,borderRadius:20}:{}),
              ...(d?{cursor:"pointer"}:{opacity:0}),
      ...
      <div style={{flex:1,overflowY:"auto",borderTop:`1px solid ${C.border}`}}>
        {selected&&selectedEvs.length===0&&(
          <div style={{padding:"20px 16px",color:C.textMuted,fontSize:13,textAlign:"center"}}>
            Нет мероприятий {selected}.{String(month+1).padStart(2,"0")}
          </div>
        )}
        {!selected&&(
          <div style={{padding:"20px 16px",color:C.textMuted,fontSize:13,textAlign:"center"}}>
            Нажми на день чтобы увидеть мероприятия
          </div>
        )}
        {selectedEvs.map(ev=>(
...
function NotesEditor({ event, onBack, onSave }) {
  ...
  const handleChange = (v) => {
    ...
    timerRef.current = setTimeout(()=>onSave(v),1500);
  };
  ...
  const handleKeyDown = (e) => {
    ...
    let prefix=null;
    if(numMatch){
      if(curLine.trim()===numMatch[2]+"."){
        e.preventDefault();
        const nv=text.slice(0,lineStart)+"\n"+text.slice(pos);
        handleChange(nv); setTimeout(()=>ta.setSelectionRange(lineStart+1,lineStart+1),0); return;
      }
    ...
    if(prefix){
      e.preventDefault();
      const nv=text.slice(0,pos)+"\n"+prefix+text.slice(ta.selectionEnd);
      handleChange(nv);
      const np=pos+1+prefix.length;
      setTimeout(()=>ta.setSelectionRange(np,np),0);
    }
  };
  ...
  return (
    <div style={sn.screen}>
      ...
      <div style={sn.scroll}>
        ...
        <textarea ref={taRef} className="nt" placeholder="Начни писать..."
          value={val} onChange={e=>{handleChange(e.target.value);autoResize();}} onKeyDown={handleKeyDown}/>
      </div>
    </div>
  );
}
...
function App() {
  ...
  if(loading) return (
    <Screen>
      <div style={s.loadingWrap}><div style={s.spinner}/><div style={{color:C.textMuted,fontSize:13,marginTop:12}}>Загружаем данные...</div></div>
    </Screen>
  );
    ...
    <Screen>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {activeEvents.length===0&&(
          <div style={s.emptyState}><div style={{fontSize:52}}>🎤</div>
    ...
    <Screen>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {archivedEvents.length===0&&(
          <div style={s.emptyState}><div style={{fontSize:52}}>🗂</div>
            <div style={{color:C.textMuted,fontSize:13}}>Архив пуст</div>
          </div>
        )}
  ...
  if(view===V.EVENT&&activeEvent) {
    ...
    return (
      <Screen>
        <TopBar
          ...
          right={
            <div style={{display:"flex",gap:6}}>
              ...
              <button style={{...s.iconBtn,borderColor:isArchived?"#A8D9BB":"#FFD9A0"}}
                className="icon-btn" onClick={()=>setArchiveConfirm(true)}>
                {isArchived?"📤":"🗂"}
              </button>
              <button style={{...s.iconBtn,borderColor:"#F5C6C2"}} className="icon-btn-del"
        ...
        {archiveConfirm&&(
          <div style={s.overlay} onClick={()=>setArchiveConfirm(false)}>
            <div style={s.confirmBox} onClick={e=>e.stopPropagation()}>
              <div style={s.confirmTitle}>{isArchived?"Восстановить?":"В архив?"}</div>
              <div style={s.confirmSub}>{isArchived?"Мероприятие вернётся в активные заказы.":"Мероприятие переместится в архив."}</div>
              <div style={{display:"flex",gap:8}}>
                <button style={s.btnGhost} onClick={()=>setArchiveConfirm(false)}>Отмена</button>
                <button style={{...s.btnSolid,background:isArchived?C.green:C.orange,flex:1}} onClick={doArchive}>
  ...
  // NOTES
  if(view===V.NOTES&&activeEvent) return (
    <NotesEditor event={activeEvent} onBack={()=>setView(V.EVENT)} onSave={updateNotes}/>
  );
          ...
          <Field label="Откуда заказ">
            <div style={s.sourceWrap}>
              {SOURCE_OPTIONS.map(opt=>(
                <button key={opt} style={{...s.sourceChip,...(draft.source===opt?s.sourceChipActive:{})}}
                  onClick={()=>setField("source",draft.source===opt?"":opt)}>{opt}</button>
              ))}
            </div>
            <input style={{...s.input,marginTop:8}} className="crm-input"
